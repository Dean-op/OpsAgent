...
class SuperBizAgentApp {
    ...
    // 格式化智能运维流式事件，保证中间过程可读
    formatAIOpsEvent(sseMessage) {
        if (!sseMessage || !sseMessage.type) return '';

        if (sseMessage.type === 'plan') {
            const planItems = Array.isArray(sseMessage.plan)
                ? sseMessage.plan.map((step, index) => `${index + 1}. ${step}`).join('\n')
                : '';
            return `\n\n## 执行计划\n\n${sseMessage.message || ''}${planItems ? `\n\n${planItems}` : ''}\n`;
        }
        ...
        if (sseMessage.type === 'status') {
            const remaining = Number.isInteger(sseMessage.remaining_steps)
                ? `（剩余 ${sseMessage.remaining_steps} 步）`
                : '';
            return `\n\n### 状态更新\n\n${sseMessage.message || ''}${remaining}\n`;
        }
    ...
    updateAIOpsStreamContent(messageElement, content) {
        ...
        if (messageContentWrapper) {
            ...
            // 流式阶段也使用 Markdown 渲染，避免换行被折叠成一整行
